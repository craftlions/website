...
- An eligible recipient is a current owner or admin whose email is verified and who is not banned. Prior login activity is irrelevant. If no eligible recipient exists when an action first becomes notifiable, no notification is created, and later verification or unbanning does not resurrect it.
...
- Delivery is durable and duplicate-resistant. A mail failure never rolls back or blocks the underlying business transition, retries stop after the mail service accepts a stable notice, and permanent failures are visible through Cloudflare operations rather than a portal admin view.
- Reminder sends occur within 24 hours of becoming due. Existing actionable phases and overdue invoices are not backfilled when the system is enabled.
