...
const NATIVE_EVIDENCE_DEPENDENCIES = Object.freeze([
	...
	{ name: "expo", platform: "adaptive", reason: "an expo dependency" },
	{
		...
		platform: "adaptive",
		reason: "a React Native metro config dependency",
	},
]);
 ...
 */
export function designSidecarCandidatesFor(
	projectRoot,
...
export function checkProduct(product, productPath = "PRODUCT.md") {
	...
	if (
		stamped === null &&
		!PRODUCT_V4_SECTIONS.some((section) => hasSection(product, section))
	) {
		findings.push(
			finding({
...
export function checkNativePlatformEvidence({
	...
	const suggested =
		platforms.size > 1 || platforms.has("adaptive")
			? "adaptive"
			: [...platforms][0];
	const declared =
		platform === "web"
...
export function checkDesignSidecar({
	...
	if (
		sidecar &&
		(schemaVersion === null || schemaVersion < DESIGN_SIDECAR_SCHEMA_VERSION)
	) {
		findings.push(
			finding({
 ...
 */
export function checkProjectRoots({
	patterns = [],
	candidates = [],
