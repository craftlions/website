...
(() => {
	...
	function checkColors(opts) {
		...
		if (classList) {
			...
			const purpleText = classStr.match(
				/\btext-(?:purple|violet|indigo)-\d+\b/,
			);
			if (
				purpleText &&
	...
	function checkGlow(opts) {
		...
		const scan = (value, prop) => {
			...
			for (const layer of value.split(/,(?![^(]*\))/)) {
				...
				const vals = extractShadowLengths(
					layer,
					colorInfo.start,
					colorInfo.end,
				);
	...
	function scanCssTextForPseudoStripe(rawContent) {
		...
		while ((m = ruleRe.exec(content)) !== null) {
			...
			const verticalCandidate =
				widthPx != null && widthPx >= 3 && widthPx <= 12;
	...
	function isScreenReaderOnlyTextStyle(style, metrics = {}) {
		...
		return (
			isAbsolutelyHidden || clippedByInset(clipPath) || clippedByRect(clip)
		);
	}
  ...
  `;
		...
		const highlight = (el, findings) => {
			...
			const entries = findings.map((f) => {
				...
				const prefix =
					RULE_CATEGORY[f.type || f.id] === "slop" ? "\u2726 " : "";
		...
		function resolvePaintedImageRect(
			...
			if (size === "cover" || size === "contain") {
				...
				if (heightToken === "auto")
					...
					paintedHeight =
						containerRect.height * (parseFloat(heightToken) / 100);
		...
		if (EXTENSION_MODE) {
			...
			window.addEventListener("message", (e) => {
				...
				if (e.data.action === "toggle-overlays") {
					const visible =
						!document.body.classList.contains("impeccable-hidden");
